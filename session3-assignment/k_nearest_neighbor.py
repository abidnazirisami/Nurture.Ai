...
class KNearestNeighbor(object):
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
        train_indices = np.argsort(dists[i])[0:k]
        
        for j in xrange(k):
            closest_y.append(self.y_train[train_indices[j]])
      #########################################################################
      # TODO:                                                                 #
      ...
      # label.                                                                #
      #########################################################################
        label_count = dict.fromkeys(list(set(closest_y)), 0)
        for j in xrange(k):
            label_count[closest_y[j]] += 1
        y_pred[i] = max(label_count, key = label_count.get)
        for key, value in label_count.items():
            if value == label_count[y_pred[i]]:
                if key < y_pred[i]:
                    y_pred[i]=key
